...
- Regenerating `.terraform.lock.hcl` without backend access: `tofu init -backend=false` (add `-upgrade` when a pin changed, especially downgrades), then `tofu providers lock -platform=linux_amd64 -platform=darwin_arm64 …`; verify the lock diff touches only the intended provider. Full recipe with auth/sandbox details: work wiki `reference_tofu-lockfile-regen`.
